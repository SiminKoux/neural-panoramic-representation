            ...
            if warmstart_label in ["uvw_mapping_f", "uvw_mapping_b"]:
                ...
                with torch.no_grad():
                    for f in range(N): 
                        ...
                        if warmstart_label == 'uvw_mapping_f':
                            uvw_f_png_folder = os.path.join(hydra.utils.get_original_cwd(), hydra.core.hydra_config.HydraConfig.get().run.dir, 'pretrain/uvw/uvw_f/png')
                            os.makedirs(uvw_f_png_folder, exist_ok=True)
                            filename = f'{uvw_f_png_folder}/iter{i+1}_{f+1}.png'
                        if warmstart_label == 'uvw_mapping_b':
                            uvw_b_png_folder = os.path.join(hydra.utils.get_original_cwd(), hydra.core.hydra_config.HydraConfig.get().run.dir, 'pretrain/uvw/uvw_b/png')
                            os.makedirs(uvw_b_png_folder, exist_ok=True)
                            filename = f'{uvw_b_png_folder}/iter{i+1}_{f+1}.png'
                        torch_utils.save_image(each_uvw_vis, filename)   
                        
                    if warmstart_label == 'uvw_mapping_f':
                        out_f_name = os.path.join(hydra.utils.get_original_cwd(), hydra.core.hydra_config.HydraConfig.get().run.dir, 'pretrain/uvw/uvw_f/mp4')
                        os.makedirs(out_f_name, exist_ok=True)
                        uvw_path = os.path.join(out_f_name, f"uvw_f_{i+1}.mp4")
                    if warmstart_label == 'uvw_mapping_b':
                        out_b_name = os.path.join(hydra.utils.get_original_cwd(), hydra.core.hydra_config.HydraConfig.get().run.dir, 'pretrain/uvw/uvw_b/mp4')
                        os.makedirs(out_b_name, exist_ok=True)
                        uvw_path = os.path.join(out_b_name, f"uvw_b_{i+1}.mp4")
            ...
            if warmstart_label == "alpha_pred":
                with torch.no_grad():
                    for f in range(N): 
                        ...
                        output_dir = os.path.join(hydra.utils.get_original_cwd(), hydra.core.hydra_config.HydraConfig.get().run.dir, 'pretrain/alpha/')
...
def infer_model(
    ...
    **kwargs
    ...
    if label == "inference":
        out_name = "inference"
        print("Inference results saving to {}".format(out_name))
    else:
...
def opt_infer_step(
    ...
    **kwargs,
    ...
    for val in range(val_epochs):
        if label == "inference":
            val_dict, val_out_dir = infer_model(step, val, label, val_loader, model, model_kwargs, **kwargs)
